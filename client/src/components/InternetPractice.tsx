...
export function InternetTrainingPage() {
  ...
  return <section className="page"><p className="eyebrow">{activeCourse.language_name} · ПРОЙДЕННЫЙ МАТЕРИАЛ</p><h1>Тренировка</h1><article className="card practice"><p>{current.lessonTitle}</p><h2>{current.word.source}</h2><form onSubmit={submit}><label>Ответ на изучаемом языке<input autoFocus value={answer} onChange={(event) => setAnswer(event.target.value)} required maxLength={500} /></label><button className="button primary" disabled={attempt.isPending}>Проверить</button></form><p role="status" aria-live="polite">{feedback}</p>{feedback && <button className="button secondary" onClick={() => { setPosition((value) => value + 1); setAnswer(""); setFeedback(""); }}>Следующее задание</button>}</article></section>;
}
