  ...
  return (
    <main className="auth-page">
      <section className="auth-card" aria-labelledby="auth-title">
        ...
        <h1 id="auth-title">{mode === "login" ? "Вход" : "Создание аккаунта"}</h1>
        ...
        <form onSubmit={submit}>
          ...
          <label>Пароль<input type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={12} maxLength={128} required value={password} onChange={(event) => setPassword(event.target.value)} /></label>
          <p className="hint">Минимум 12 символов. Не используйте пароль от другого сервиса.</p>
          <button className="primary" disabled={busy} type="submit">{busy ? "Подождите…" : mode === "login" ? "Войти" : "Создать аккаунт"}</button>
