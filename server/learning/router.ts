...
export function createLearningRouter(pool:pg.Pool){const router=express.Router(),families=new FamilyService(pool),learning=new LearningService(pool);
  ...
  router.get("/enrollments",async(_req,res)=>res.json({enrollments:await learning.enrollments(res.locals.auth.userId,res.locals.auth.familyId)}));
  router.get("/settings",async(_req,res)=>res.json(await learning.settings(res.locals.auth.userId,res.locals.auth.familyId)));
  router.put("/settings",async(req,res)=>res.json(await learning.saveSettings(res.locals.auth.userId,res.locals.auth.familyId,z.record(z.unknown()).parse(req.body))));
