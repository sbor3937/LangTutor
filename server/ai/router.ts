...
export function createAiRouter(pool:pg.Pool){const router=express.Router(),families=new FamilyService(pool),repository=new AiUsageRepository(pool),gateway=new AiGateway(repository,new Map<string,TutorProvider>([["demo",new DemoTutorProvider()],["openrouter",new OpenRouterTutorProvider()]]));
